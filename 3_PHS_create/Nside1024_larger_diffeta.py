...
SW_angle_arr = np.loadtxt(f'SW_stack_eta{eta}.txt')
ISW_angle_arr = np.loadtxt(f'ISW_stack_eta{eta}.txt')
...
Signal_Map = np.empty(( 0,  Map_dimY, Map_dimX  ))

for trials in range(num_sigs):
    print(trials)

    r1 = randint(1,49) # Spot 1 depth
    #r1 = randint(16,43)  # Spot 1 depth constrained
    #r1 = 25

    for rep in range(20000000000000):
        r2 = randint(1,50)
        #r2 = 25

        deltax = randint(-50,50)
        deltay = randint(-50,50)
    ...
    framedimY = Map_dimY
    framedimX = Map_dimX
    frame = np.zeros((framedimY, framedimX))
    
    random_shiftX = randint(1,framedimX-PHS_shape[1]-10)
    random_shiftY = randint(1,framedimY-PHS_shape[0]-10)

    print("Random shift = ("+str(random_shiftY)+", "+str(random_shiftX)+")")

    frame[random_shiftY:random_shiftY+PHS_signal.shape[0],random_shiftX:random_shiftX+PHS_signal.shape[1]]+=PHS_signal
    
    frame = np.reshape(frame, (1, Map_dimY, Map_dimX) )
    
    Signal_Map = np.vstack((Signal_Map, frame))



file_loc = '/afs/crc.nd.edu/user/t/tkim12/Work/CMB_ML/Data/Nside1024/test_img/sig_larger/'
file_name = str(num_sigs)+'_PHS_'+str(Map_dimY)+'sq'
np.save(file_loc+file_name+'eta_'+str(eta)+'_g1_Sig_offLSS_'+str(batch_num), Signal_Map)
#print(Signal_Map.shape)
#plt.imshow(Signal_Map[0])
#plt.show()
